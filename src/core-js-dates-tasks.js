...
function getNextFriday(date) {
  ...
  data.setDate(data.getDate() + numberOfDay);
...
function isDateInPeriod(date, period) {
  ...
  const dateInspect = new Date(date);

  return (
    dateStart.getTime() <= dateInspect.getTime() &&
    dateEnd.getTime() >= dateInspect.getTime()
  );
}
...
function getWeekNumberByDate(date) {
  ...
  const difference = (date - data) / (3600 * 1000 * 24);
  return Math.ceil((difference + data.getDay()) / 7);
}
...
function getNextFridayThe13th(date) {
  const tempDate = new Date(date.setDate(13));
  for (let m = 0; m < 12; m += 1) {
    const iterateDate = new Date(tempDate.setMonth(m));
    if (iterateDate.getDay() === 5) return iterateDate;
  }
  return null;
}
